 ...
 * Use <code>Protocol</code> as an example, its configuration file 'META-INF/letter/com.xxx.Protocol' is changed from: <br/>
 * <pre>
 *     com.foo.XxxProtocol
 *     com.foo.YyyProtocol
 * </pre>
 ...
 * fail to initialize if the third party library doesn't exist. In this case, letter cannot figure out extension's id
 * therefore cannot be able to map the exception information with the extension, if the previous format is used.
 * <p/>
 * For example:
 * <p>
 * Fails to load Extension("mina"). When user configure to use mina, letter will complain the extension cannot be loaded,
